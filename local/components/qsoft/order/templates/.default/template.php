...
<div id="main-basket-block">
    ...
    <? endif ?>
    <? if (!empty($arResult['ITEMS']['LOCAL']) && !empty($arResult['ITEMS']['NOT_LOCAL'])) : ?>
    <div class="two-baskets-conditions col-xs-12">
        <p>
        <?= COption::GetOptionString("respect", "text_for_double_basket") ?>
        </p>
    </div>
    <? elseif (!empty($arResult['ITEMS']['NOT_LOCAL'])) : ?>
    <div class="two-baskets-conditions col-xs-12">
        <p>
        <?= COption::GetOptionString("respect", "text_for_basket_without_local_products") ?>
        </p>
    </div>
    <? endif;?>
...
?>
    ...
    <? endif; ?>
    <? if (!empty($arResult['ITEMS']['NOT_LOCAL'])) : ?>
    <div id="full_basket-2" class="col-xs-12 col-xs-12--padding-sm-0 full_basket">
            <div class="main main--banner full_basket-container--regional">
                <div class="checkout <?=empty($arResult['ITEMS']['LOCAL']) ? '' : 'basket-border'?>">
                    <? if (!empty($arResult['ITEMS']['LOCAL'])) : ?>
                    <div class="opening-cart">
                        <svg class="plus">
                            <use xlink:href="/local/templates/respect/icons/icons-sprite.svg#plus"></use>
                        </svg>
                        <?= COption::GetOptionString("respect", "text_for_popup_basket_without_local_products") ?>
                    </div>
                    <? endif; ?>
                    <form id="b-order2" class="checkout__inner clearfix" action="<?= $APPLICATION->GetCurPageParam() ?>" style="<?=empty($arResult['ITEMS']['LOCAL']) ? 'display: block' : ''?>"  method="post">
                        <!-- main -->
                        <div class="checkout__col checkout__col--main col-md-8">
                            <!-- orders -->
                            <div class="checkout__orders orders">
                                ...
                                <!-- /orders header row -->
                                <div id="orders__row-container2">
                                    <? if ($this->__component->checkType(array("offers2"))) : ?>
                                        <? $APPLICATION->RestartBuffer() ?>
                                    <? endif ?>
                                    <!-- orders cards rows -->
                                    <? foreach ($arResult['ITEMS']['NOT_LOCAL'] as $id => $arItem) : ?>
                                        <? $arResult["BASKET"][] = [
                                        ...
                                        $propCat .= $arItem['SUBTYPEPRODUCT'] ? '/' . $arItem['SUBTYPEPRODUCT'] : '';
?>
                                        <div class="flex-product orders__row orders__row--product2 <?= $arResult["PROBLEM_NOT_LOCAL_OFFERS"][$id] ? "orders__row--product--error2" : "" ?> js-card"
                                             data-prod-id="<?= $arItem['PRODUCT_ID'] ?>"
                                             data-prod-articul="<?= $arItem['ARTICLE'] ?>"
                                             data-prod-name="<?= $arItem['NAME'] . ($arItem['ARTICLE'] ? ' | ' . $arItem['ARTICLE'] : '') ?>"
                                             data-prod-brand="<?= $arItem['BRAND'] ?>"
                                        ...
                                        >
                                            ...
                                            <div class="flex-product--size select-size-container">
                                                <? if (!empty($arItem['AVAILABLE_SIZES'])) : ?>
                                                    <div class="orders__col--select-size-container"
                                                         id="select-size-container2">
                                                        <select name="select-size2" id="select-size-<?= $id ?>">
                                                            <option value="<?= $id ?>"
                                                                    selected="selected"><?= $arItem['SIZE'] ?></option>
                                                            <? foreach ($arItem['AVAILABLE_SIZES'] as $size => $offerId) : ?>
                                                                <option value="<?= $offerId ?>"><?= $size ?></option>
                                                    ...
                                                    <div class="orders__col orders__col--add-btn-container">
                                                        <button id="buy-btn-<?= $id ?>" class="orders__add-btn2"
                                                                type="button" value="<?= $id ?>">
                                                            Добавить размер
                                                        </button>
                                                    </div>
                                            ...
                                            <div class="flex-product--count orders__col">
                                                <span class="orders__label--only-mobile">Кол-во:</span>
                                                <input type="text" class="orders__count-input" name="count" value="1"
                                                       disabled>
                                            </div>
                                            <? //количество end ?>

                                            <? //стоимость ?>
                                            <div class="orders__col orders__col--price"
                                                 style="display: none!important;">
                                                <span class="orders__label">Стоимость:</span>
                                                <span class="orders__price2">
                                                <span class="orders__price-num2"
                                                      data-price="<?= $arItem['PRICE'] ?>"><?= number_format($arItem['PRICE'], 0, "", "&nbsp;") ?>&nbsp;р.</span>
                                                <? if ($arItem['OLD_PRICE'] && $arItem['PRICE'] < $arItem['OLD_PRICE']) : ?>
                                                    <s><span class="orders__old-price-num2"
                                                             data-price="<?= $arItem['OLD_PRICE'] ?>"><?= number_format($arItem['OLD_PRICE'], 0, "", "&nbsp;") ?>&nbsp;р.</span></s>
                                                <? endif ?>
                                            </span>
                                            </div>
                                            ...
                                            <a class="orders__remove js-card-remove2"
                                               data-product-id="<?= $arItem["PRODUCT_ID"] ?>"
                                               data-id="<?= $id ?>"
                                               title="Удалить"></a>
                                        </div>
                            ...
                            <div class="left-cart-block-not-local">
                            <!-- /orders -->
                            <? if (empty($arResult["ERRORS"]['NOT_LOCAL'])) : ?>
                                <!-- delivery -->
                                <div class="checkout__block checkout__block--delivery">
                                    <h2 class="checkout__title checkout__title--delivery">Доставка <span class="err-order err-delivery"></span></h2>
                                    <? if (!empty($arResult["WAYS_DELIVERY"]['NOT_LOCAL'])) : ?>
                                        <div class="checkout__block checkout__block--flex">
                                            <? foreach ($arResult["WAYS_DELIVERY"]['NOT_LOCAL'] as $arDeliveryWay) : ?>
                                                <div class="form__box form__box--1-2">
                                                    <div class="cart-delivery">
                                                        <div class="cart-delivery__wrapper">
                                                            <?//Указывается нулевой индекс потому что несколько элементов прилетает только для ПВЗ, а логика их выбора описана в компоненте карты ПВЗ?>
                                                            <input id="delivery_<?= $arDeliveryWay['DELIVERY'][0] ?>" class="<?=$arDeliveryWay['PVZ'] ? 'is-pvz2 ' : ''?>checkbox4 js-delivery2 cart-delivery__input" type="radio" name="DELIVERY"
                                                                   data-allowed-payments="<?=implode(',', $arDeliveryWay['ALLOWED_PAYMENTS'])?>"
                                                                    <? if ($arDeliveryWay['PVZ']) : ?>
                                                                        <? foreach ($arDeliveryWay['ALLOWED_PVZ_PAYMENTS'] as $class => $paymentsIds) :?>
                                                                            data-allowed-payments-<?=$class?>="<?=implode(',', $paymentsIds)?>"
...
<!--                                                                </div>-->
                                ...
                                <div class="checkout__form2">
                                    <input type="hidden" name="action" value="order">
                                    <input type="hidden" name="PROPS[IS_LOCAL]" value="N" hidden>
                                    <div class="checkout__form-fields form">
                                        <div class="form__errors" id="form__errors-block2"></div>
                                        <div class="form__field form__field--payment">
                                            <div class="form__box">
                                                <div class="checkout__block--payment">
                                                    <h2 class="checkout__title checkout__title--delivery">Оплата <span class="err-order err-payment"></span></h2>
                                                    <div class="checkout__block--flex">
                                                    <? foreach ($arResult["WAYS_PAYMENT"]['NOT_LOCAL'] as $arPaymentWay) :?>
                                                        <div class="form__box form__box--1-2 payment__type2 payment__type--disabled">
                                                        <div class="cart-delivery">
                                                            <div class="cart-delivery__wrapper">
                                                                <input id="Payment_<?= $arPaymentWay['PAYMENT'] ?>"
                                                                       class="checkbox4 cart-delivery__input js-payment-not-local"
                                                                       type="radio"
                                                                       name="PAYMENT"
                                                                       data-sort="<?=$arPaymentWay['SORT']?>"
                                                                       data-prepayment="<?=$arPaymentWay['PREPAYMENT']?>"
                                        ...
                                        <div class="checkout__block--contact-info">
                                            <div class="form__box">
                                                <div class="form__field">
                                                    <h2 class="checkout__title checkout__title--contact-info">Контактные данные</h2>
                                                    <input class="form__elem js-required js-fio" type="text" name="PROPS[FIO]" value="<?= (trim($arResult["USER"]["NAME"]." ".$arResult["USER"]["SECOND_NAME"]." ".$arResult["USER"]["LAST_NAME"])) ?: $arResult["COOKIE_FIO"][1] ?>" placeholder="*Ф.И.О.">
                                                    <div class="err-order err-PROPS[FIO]"></div>
                                                </div>
                                                <div class="form__field form__field--1-2">
                                                    <input class="form__elem js-required js-email" type="text" name="PROPS[EMAIL]" value="<?= !preg_match('`.*@rshoes.ru`i', $arResult["USER"]["EMAIL"] ?: $arResult["COOKIE_EMAIL"][1]) ? ($arResult["USER"]["EMAIL"] ?: $arResult["COOKIE_EMAIL"][1]) : '' ?>" placeholder="*E-Mail">
                                                    <div class="err-order err-PROPS[EMAIL]"></div>
                                                </div>
                                                <div class="form__field form__field--1-2">
                                                    <input class="form__elem js-required js-phone2" type="text" name="PROPS[PHONE]" value="<?= $arResult["USER"]["PERSONAL_PHONE"] ?: $arResult["COOKIE_PHONE"][1] ?>" placeholder="*Телефон">
                                                    <div class="err-order err-PROPS[PHONE]"></div>
                                                </div>
                                                <? $APPLICATION->IncludeComponent(
                                                    'qsoft:subscribe.manager',
                                                ...
                                                <? $APPLICATION->IncludeComponent(
                                                    ...
                                                    array(
                                                        'CACHE_TYPE' => 'A',
                                                        'CACHE_TIME' => 31536000,
                                                        'BASKET_TYPE' => 'NOT_LOCAL',
                                                    )
                                                ); ?>
                                                <? if ($arResult["DELIVERY"]["PVZ"]) : ?>
                                                    <div class="form__field form__field--1-2 js__cdek-enabled2 is-hidden">
                                                        <input id="cart__delivery-cdek-button2" class="form__elem" type="button" value="Выбрать пункт выдачи заказов">
                                                        <input id="cart__delivery-cdek-input2" class="form__elem js-required" type="hidden" name="PROPS[PVZ_ID]" value="" placeholder="*Пункт выдачи">
                                                    </div>
                                                <? endif ?>
                                                <div class="form__field form__field--1-2 js__cdek-disabled2 js-dadata-street2">
                                                    <input id="street_user2" class="form__elem js-required" type="text" name="PROPS[STREET_USER]" value="<?= $arResult["USER"]["PERSONAL_STREET"] ?>" placeholder="*Улица">
                                                    <div class="err-order err-PROPS[STREET_USER]"></div>
                                                </div>
                                                <div class="form__field form__field--1-2 js__cdek-disabled2 js-dadata-house2">
                                                    <input id="house_user2" class="form__elem js-required" type="text" name="PROPS[HOUSE_USER]" value="<?= $arResult["USER"]["UF_HOUSE"] ?>" placeholder="*Дом, корпус, строение">
                                                    <div class="err-order err-PROPS[HOUSE_USER]"></div>
                                                </div>
                                                <div class="form__field form__field--1-2 js__cdek-disabled2">
                                                    <input <?=$arResult['DADATA_STATUS'] && COption::GetOptionInt("likee", "dadata_active") ? 'readonly ' : ''?>id="postal_code2"  class="form__elem" type="number" name="PROPS[POSTALCODE]" value="<?= $arResult["USER"]["UF_POSTALCODE"] ?>" placeholder="<?=$arResult['DADATA_STATUS'] && COption::GetOptionInt("likee", "dadata_active") ? 'Индекс, заполняется автоматически' : 'Индекс (не обязательно)'?>">
                                                </div>
                                                <div class="form__field form__field--1-4 js__cdek-disabled2">
                                                    <input id="flat2" class="form__elem" type="text" name="PROPS[FLAT]" value="<?= $arResult["USER"]["UF_APARTMENT"] ?>" placeholder="Кв/офис">
                                                </div>
                                                <div class="form__field form__field--1-4 js__cdek-disabled2">
                                                    <input id="porch2" class="form__elem" type="text" name="PROPS[PORCH]" value="<?= $arResult["USER"]["UF_ENTRANCE"] ?>" placeholder="Подъезд">
                                                </div>
                                                <div class="form__field form__field--1-4 js__cdek-disabled2">
                                                    <input id="floor2" class="form__elem" type="text" name="PROPS[FLOOR]" value="<?= $arResult["USER"]["UF_FLOOR"] ?>" placeholder="Этаж">
                                                </div>
                                                <div class="form__field form__field--1-4 js__cdek-disabled2">
                                                    <input id="intercom2" class="form__elem" type="text" name="PROPS[INTERCOM]" value="<?= $arResult["USER"]["UF_INTERCOM"] ?>" placeholder="Домофон">
                                                </div>
                                                <? foreach ($arResult['DADATA_PROPS'] as $dadataProp) :?>
                                                    <div class="form__field js__cdek-disabled" hidden>
                                                        <input hidden id="<?= $dadataProp ?>2"  class="form__elem" type="text" name="PROPS[<?= mb_strtoupper($dadataProp) ?>]" value="<?= $arResult["USER"]["UF_" . mb_strtoupper($dadataProp)] ?>" placeholder="Заполняется автоматически">
                                                    </div>
                                                <? endforeach; ?>
                                                <div class="clear-blocks"></div>
                                            </div>
                        ...
                        <!-- /main -->
                        <!-- sidebar -->
                        <div class="right-cart-block-not-local checkout__col checkout__col--sidebar col-md-4">
                            <? if (empty($arResult["ERRORS"]['NOT_LOCAL'])) : ?>
                                <!-- promocode -->
                                <div class="checkout__block">
                                    <h2 class="checkout__title">Промокод</h2>
                                    <div class="form__box">
                                        <div class="form__field">
                                            <input id="cart__coupon2" class="form__elem" type="text" name="COUPON" value="<?= ($arResult["COUPON"] && $arResult["DISCOUNT"]) ? $arResult["COUPON"] : "" ?>" placeholder="Введите промокод при его наличии">
                                        </div>
                                        <div id="cart__coupon-error2" class="form__field"></div>
                                        <div class="form__field">
                                            <input id="cart__coupon-button2" type="button" class="form__btn bttn bttn--yellow" value="ПРИМЕНИТЬ ПРОМОКОД">
                                        </div>
                                    </div>
                                </div>
                                <!-- /promocode -->
                                ...
                                <div class="checkout__block">
                                    ...
                                    <div class="form__box">
                                        <div class="form__field">
                                            <div class="p">
                                                <div class="l">Доставка</div>
                                                <div id="cart__delivery-price2" class="r">Способ доставки не выбран</div>
                                            </div>
                                            <div id="cart__discount-block2" class="p <?= !$arResult["DISCOUNT"]['NOT_LOCAL'] ? "is-hidden" : "" ?>">
                                                <div class="l">Скидка</div>
                                                <div id="cart__discount-price2" class="r"><?= number_format($arResult["DISCOUNT"]['NOT_LOCAL'], 0, "", "&nbsp;") ?>&nbsp;р.</div>
                                            </div>
                                            <div class="p">
                                                <div class="l">Всего к оплате</div>
                                                <div id="cart__total-price2" class="r"><?= number_format($arResult["PRICE"]['NOT_LOCAL'], 0, "", "&nbsp;") ?>&nbsp;р.</div>
                                            </div>
                                        </div>
                                        <div class="form__field">
                                            <input id="cart__order-button2" class="form__btn bttn" type="submit" name="submit" value="Отправить заказ">
                                        </div>
                                        <div id="basket_checkbox_policy2" class="col-xs-12">
                                            <div class="err-order err-policy"></div>
                                            <input id="cart__order-policy2" type="checkbox" name="basket_checkbox_policy" class="checkbox3 js-required" checked="">
                                            <label for="cart__order-policy2" class="checkbox--_">Я соглашаюсь на обработку моих персональных данных и ознакомлен(а) с <a href="<?= OFFER_FILENAME ?>">политикой конфиденциальности</a>.</label>
                                        </div>
                                    </div>
                                </div>
                                <!-- /cost -->
                            <? else : ?>
                                <div class="checkout__error-wrapper2">
                                    <? foreach ($arResult["ERRORS"]['NOT_LOCAL'] as $error) : ?>
                                        <p class="checkout__error-text"><?= $error ?></p>
                                    <? endforeach ?>
                                </div>
                            <? endif ?>
