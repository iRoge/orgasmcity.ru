    ...
    <? foreach ($arResult['ITEMS'] as $arItem) :
        if ($arItem['ACTIVE'] == 1) : ?>
            <p class="<?= $arItem['COLLAPSE'] ? '' : 'active-';?>blue sectionEvent infopage-section">
                <?= $arItem['NAME'] ?>
                <img src="<?= SITE_TEMPLATE_PATH; ?>/img/arr-up.png" <?= $arItem['COLLAPSE'] ? '' : 'style="display:block;"'; ?> class="arr-up"/>
                <img src="<?= SITE_TEMPLATE_PATH; ?>/img/arr-dwn.png" <?= $arItem['COLLAPSE'] ? '' : 'style="display:none;"'; ?>class="arr-down"/>
            </p>
            <div style="<?= $arItem['COLLAPSE'] ? '' : 'display:block;'; ?>margin-top:0" class="after-blue"><?= $arItem['TEXT'] ?></div>
        <? endif; ?>
    <? endforeach; ?>
<? endif; ?>