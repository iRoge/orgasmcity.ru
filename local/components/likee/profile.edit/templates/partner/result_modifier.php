...
if (isset($_REQUEST['region_name'])) {
    $sRegion = htmlentities(trim($_REQUEST['region_name']));
} elseif (!empty($arResult['PROFILE']['FIELDS']['REGION']['VALUE'])) {
    $sRegion = $arResult['PROFILE']['FIELDS']['REGION']['VALUE'];
} else {
    $sRegion = $arResult['PROPS']['REGION']['VALUE'];
}
...
foreach ($arResult['PROPS'] as $sKey => $arProp) {
    $arResult['GROUPS'][$arProp['PROPS_GROUP_ID']]['PROPS'][$sKey] = $arProp;
}