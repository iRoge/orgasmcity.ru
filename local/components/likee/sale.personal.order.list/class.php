...
class CBitrixPersonalOrderListComponent extends CBitrixComponent
{
    ...
    protected function obtainDataOrders()
    {
        ...
        while ($basket = $listBaskets->fetch()) {
            ...
            $arOffersXMLId[] = $basket['PRODUCT_ID'];
        }

        $this->products = $this->getOffers($arOffersXMLId);
    ...
    protected function formatResult()
    {
        ...
        $arResult['PRODUCTS'] = $this->products;
    ...
    private function getOffers($arOffersId)
    {
        $arSelect = array(
            "ID",
            "IBLOCK_ID",
        ...
        while ($arItem = $res->Fetch()) {
            ...
            $arProdsId[] = $arItem['PROPERTY_CML2_LINK_VALUE'];
        }

        $arProds = $this->getProducts($arProdsId);

        foreach ($arOffersNew as &$value) {
            $value = $value + $arProds[$value['PROPERTY_CML2_LINK_VALUE']];
        }
