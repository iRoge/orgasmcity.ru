...
/**
 * Class RestrictionByLocation
 * Restricts pay system by location(s)
 */
class RestrictionByLocation extends Restriction
{
    ...
    {
        return Loc::getMessage('DESCRIPTION');
    }

    /**
     * This function should accept only location CODE, not ID, being a part of modern API
     */
    public static function check($locationCode, array $restrictionParams, $pay_system_id = 0)
    {
        if (intval($pay_system_id) <= 0) {
            return true;
        }
    ...
    protected static function prepareParamsForSaving(array $params = array(), $pay_system_id = 0)
    {
        if ($pay_system_id > 0) {
            $arLocation = array();
            ...
            PaySystemLocationTable::resetMultipleForOwner($pay_system_id, $arLocation);
        }

        return array();
    }

    public static function getParamsStructure($pay_system_id = 0)
    {

        $result =  array(
            "LOCATION" => array(
                "TYPE" => "PAY_SYSTEM_LOCATIONS"
            )
        );

        if ($pay_system_id > 0) {
            $result["LOCATION"]["PAY_SYSTEM_ID"] = $pay_system_id;
        }
    ...
    public static function delete($restrictionId, $pay_system_id = 0)
    {
        PaySystemLocationTable::resetMultipleForOwner($pay_system_id);
    ...
    protected static function getLocationsCompat(array $restrictionFields, $leftMargin, $rightMargin)
    {
        ...
        $res = PaySystemLocationTable::getList(array(
            'filter' => array(
                '=PAY_SYSTEM_ID' => array_keys($restrictionFields),
                array(
                    'LOGIC' => 'OR',
                    array(
                        'LOGIC' => 'AND',
        ...
        while ($d2l = $res->fetch()) {
            if ($d2l['LOCATION_TYPE'] == Connector::DB_LOCATION_FLAG) {
                ...
                if (!in_array($d2l['PAY_SYSTEM_ID'], $result[$d2l['LOCATION_CODE']])) {
                    $result[$d2l['LOCATION_CODE']][] = $d2l['PAY_SYSTEM_ID'];
                }
            } else {
                ...
                if (!in_array($d2l['PAY_SYSTEM_ID'], $groups[$d2l['LOCATION_CODE']])) {
                    $groups[$d2l['LOCATION_CODE']][] = $d2l['PAY_SYSTEM_ID'];
                }
            }
        }
