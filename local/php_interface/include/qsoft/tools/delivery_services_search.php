...
$arDeliveriesIds = WaysByDeliveryServicesTable::getList([
   'select' => ['DELIVERY_ID'],
])->fetchAll();

$arRequestIds = explode(',', $_REQUEST['id']);
$arDeliveriesIds = array_column($arDeliveriesIds, 'DELIVERY_ID');
...
<script type="text/javascript">
    function SetValue(id, name, desc) {
        ...
        let idBlock = window.opener.document.getElementById('id<?=$_REQUEST['num']?>');
        if (idBlock) {
            idBlock.innerHTML = id;
        }
