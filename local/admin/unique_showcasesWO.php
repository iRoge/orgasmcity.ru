...
$APPLICATION->SetTitle('Уникальные витрины без учета работы складов');
require_once($_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/main/include/prolog_admin_after.php');

$cache = new CPHPCache();

if ($cache->InitCache(604800, 'unique_showcases_wo', '/local/admin')) {
    $arResultWO = $cache->GetVars()['result'];
    $arStores = $cache->GetVars()['stores'];
} elseif ($cache->StartDataCache()) {
    $CACHE_MANAGER->StartTagCache('/local/admin');
    $CACHE_MANAGER->RegisterTag('unique_showcases_wo');
    ...
    $query = 'SELECT * FROM rdevs_uniqshowcase_wo';
...
if (isset($_POST["is_ajax_download"])) {
    ...
    $count = 1;
    $xlsRow = 3;
    foreach ($arResultWO as $showcaseId => $arItem) {
        $xlsRowMerge = $xlsRow;
        ...
        foreach ($arItem['STORES'] as $storeId) {
    ...
    $sheet->getStyle("B3:F" . $xlsRow)->getAlignment()
        ->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
    $sheet->getStyle("H3:G" . $xlsRow)->getAlignment()
        ->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);

    $writer = new Xlsx($spreadsheet);
    $uploaddir = COption::GetOptionString("main", "upload_dir", "upload");

    $filename = "/" . $uploaddir . "/tmp/unique_showcasesWO.xlsx";
...
?>
    ...
    <button id="refresh_uniqshowcase-wo">Пересчитать уникальные витрины без учета работы складов</button>
    ...
    <table class="unique-showcases">
        ...
        <?
        $count = 1;
        foreach ($arResultWO as $showcaseIdWO => $arItem) : ?>
            <tr>
            <td rowspan="<?= count($arItem['STORES']) ?>"><?= $count ?></td>
            <td rowspan="<?= count($arItem['STORES']) ?>"><?= $arItem['UNIQ_ID'] ?></td>
            <td rowspan="<?= count($arItem['STORES']) ?>"><?= $showcaseIdWO ?></td>
    ...
    <script>
        ...
        $("#refresh_uniqshowcase-wo").on("click", function () {
            $.ajax({
                ...
                data: {'with_store_work_type': 'no', 'value': "true"},
                success: function (data) {
