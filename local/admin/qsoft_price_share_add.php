...
if (!empty($_POST)) {
    ...
    if (!empty($_POST['action']['change_list'])) {
        ...
        if (empty($errors)) {
            ...
            $changes['UF_CREATION_DATE'] = date("d.m.Y H:i:s");
            ...
            if ($_POST['action']['change_share'] == 'Сохранить') {
                if (!empty($changes)) {
                    $hlblock = HLBT::getList(array('filter' => array('=NAME' => 'PriceShare')))->fetch();
                    $entity = HLBT::compileEntity($hlblock);
                    $entity_data_class = $entity->getDataClass();
                    $result = $entity_data_class::add($changes);
                    if ($result) {
                        header("Location: http://" . $_SERVER['HTTP_HOST'] . "/bitrix/admin/qsoft_price_share_list.php?lang=ru&success=added&bad_articles=" . implode(',', $badArticles));
                    } else {
                        $errors['main'][] = 'Произошла ошибка сохранения в бд';
                    }
                }
            }
        }
...
<div style="width: auto;max-width: 800px" class="adm-detail-content">
    <div style="width: auto; display: inline-block;" class="adm-detail-content-item-block">
        ...
        <?php elseif (!empty($sharesData) || $success) : ?>
                ...
                <div class="adm-info-message">
                    <div class="adm-info-message-title">
                        ...
                        <?php if ($_POST['action']['change_list'] == 'Добавить') : ?>
                            <p>Артикулы добавлены в список</p>
                        <?php endif;?>
        ...
        <?php endif; ?>
        <form method="POST" action="/bitrix/admin/qsoft_price_share_add.php?lang=ru" enctype="multipart/form-data" name="add_price_share" id="add_price_share">
            <table class="main-share-options">
                ...
                <tr>
                    ...
                    <td>
                        <select name="share_type" onchange="changePriceSegment(this)">
                ...
                <tr>
                    ...
                    <td>
                        <input name="segment" type="text" value="<?=!empty($_POST) ? $_POST['share_type'] == 1 || $_POST['share_type'] == 2 ? 'Red' : 'White' : 'Red'?>" readonly>
                    </td>
                </tr>
                </tbody>
            </table>
            <? if ($_POST['action']['change_list'] || $_POST['action']['change_share']) : ?>
        ...
        <table style="width: 450px">
            ...
            <tr>
                <td>
                    <input style="position: absolute;" type="file" name="import" form="add_price_share"/>
                </td>
                <td>
                    <input  style="margin-top: 20px" class="adm-btn-save" name="action[change_list]" type="submit" value="Добавить" title="Добавить артикулы из файла (одинаковые артикулы заменятся на новые)" form="add_price_share">
                    <input  style="margin-top: 20px" class="adm-btn-save" name="action[change_list]" type="submit" value="Поменять" title="Полностью поменять на артикулы из файла" form="add_price_share">
                </td>
            </tr>
        </table>
        <div style="padding-top: 75px" class="share-buttons">
            <input class="adm-btn-save" name="action[change_share]" type="submit" value="Сохранить" title="Создать акцию" form="add_price_share">
            <input type="button" value="Отменить" name="cancel" onclick='top.window.location="/bitrix/admin/qsoft_price_share_list.php?lang=ru"' title="Вернуться к списку">
        </div>
    </div>
</div>
<script>
    ...
    function confirmDelete() {
        if (confirm("Вы точно хотите удалить акцию?")) {
            top.window.location="/bitrix/admin/qsoft_price_share_list.php?lang=ru&delete_share_id=<?=$arShare['ID']?>"
        } else {
            return false;
        }
    }
