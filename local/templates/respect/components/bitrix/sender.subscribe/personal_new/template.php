...
global $USER;
?>
<div class="bx-subscribe main_container_subscribe"  id="sender-subscribe">
<div class="content_top">
<p>УПРАВЛЕНИЕ РАССЫЛКОЙ</p>
<p><?=$USER->GetEmail()?></p>
<p>ПОДПИШИТЕСЬ НА РАССЫЛКУ RESPECT, ЧТОБЫ БЫТЬ В КУРСЕ ВСЕХ АКЦИЙ И НОВОСТЕЙ</p>

<div>
  <input type="checkbox" id="check" name="check" checked/>
  <label for="check">Я хочу получать рассылку Respect</label>
</div>
</div>
<?
...
?>
	...
	<form id="bx_subscribe_subform_<?=$buttonId?>" role="form" method="post" action="<?=$arResult["FORM_ACTION"]?>">
		...
		<div class="bx-input-group" style="display: none;">
			<input class="bx-form-control" type="email" name="SENDER_SUBSCRIBE_EMAIL" value="<?=$USER->GetEmail()?>" title="<?=GetMessage("subscr_form_email_title")?>" placeholder="<?=htmlspecialcharsbx(GetMessage('subscr_form_email_title'))?>">
		</div>
