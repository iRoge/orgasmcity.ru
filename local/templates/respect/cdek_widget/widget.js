function ipolWidgetController(setups) {
...
function ISDEKWidget(params) {

	if (!params.path) {
		var scriptPath = document.getElementById('widget_script').src;
		scriptPath = scriptPath.substring(0, scriptPath.indexOf('widget.js')) + 'scripts/';
	...
	var loaders = {
		onJSPCSSLoad: function () {
			widget.states.check('JSPCSS');
		},
		onStylesLoad: function () {
			widget.states.check('JSPCSS');
			widget.states.check('styles');
		},
		onIPJQLoad: function () {
			widget.states.check('jquery')
		},
		onJSPJSLoad: function () {
			widget.states.check('JSPJS')
		},
		onPVZLoad: function () {
			widget.states.check('PVZ')
		},
		onDataLoad: function () {
			widget.states.check('data')
		},
		onLANGLoad: function () {
			widget.states.check('lang')
		},
		onYmapsLoad: function () {
			widget.states.check('ymaps');
		},
		onYmapsReady: function () {
			widget.states.check('mapsReady')
		},
		onYmapsInited: function () {
			widget.states.check('mapsInited')
		},
		onCityFrom: function () {
			widget.states.check('cityFrom')
		}
	};

	var widget = new ipolWidgetController({
		label: 'ISDEKWidget',
		options: {
			path: {
				value: params.path,
			...
			link: {
				...
				setting: 'afterJquery',
				hint: 'No element whit this id to put the widget'
			},
			defaultCity: {
		...
		stages: {
			...
			'afterJquery': {
				...
				function: function () {
					...
					ipjq.getJSON(
						widget.options.get('servicepath'),
			...
			'dataLoaded': {
				...
				function: function () {
					loaders.onDataLoad();
					if (widget.options.get('cityFrom')) {
						ipjq.getJSON(
							widget.options.get('servicepath'),
							{isdek_action: 'getCity', city: widget.options.get('cityFrom')},
							function (data) {
								if (typeof(data.error) === 'undefined') {
									CALCULATION.cityFrom = data.id;
								} else {
									widget.logger.warn("City from was't found " + data.error);
								}
			...
			'ready': {
				...
				function: function () {
                    if (widget.options.get('defaultCity') != "auto")
                        DATA.city.set(widget.options.get('defaultCity'));
					template.readyA = true;
					template.html.loadCityList(DATA.city.collection);
					if (!widget.popupped) {
						widget.finalAction();
					} else {
						widget.loadedToAction = true;
					}
	...
	widget.popupped = false;
	widget.loadedToAction = false;
	widget.finalActionCalled = false;
	widget.loaderHided = false;

	widget.finalAction = function () {
		if (widget.finalActionCalled === true) {
			return;
		}
		widget.finalActionCalled = true;
	...
	widget.hideLoader = function () {
		if (!widget.loaderHided) {
			widget.loaderHided = true;
	...
	function YmapsLoader() {
		if (typeof (widget.incrementer) === 'undefined') {
			widget.incrementer = 0;
		}
		if (typeof(ymaps.geocode) !== 'function') {
			if (widget.incrementer++ > 50) {
				widget.logger.error('Unable to load ymaps');
			} else {
		...
		city: {
			...
			set: function (intCityID) {
				...
				if (this.checkCity(intCityID)) {
					...
					widget.logger.error('Unknown city: ' + intCityID);
					return false;
				}
			},
		...
		PVZ: {
			...
			getCityPVZ: function (intCityID) {
				...
				if (this.check(intCityID)) {
					...
					widget.logger.error('No PVZ in city ' + intCityID);
				}
			},
	...
	var CALCULATION = {
		...
		calculate: function () {
			if (this.cityFrom) {
				...
				widget.logger.warn('No city from given: calculation is impossible');
			}
		},
		...
		onCalc: function (answer) {
			...
			if (typeof(answer.error) !== 'undefined') {
				...
				if (thisIsNorma) {
					widget.logger.warn('Troubles while calculating: ' + sign.substring(0, sign.length - 2));
					...
					widget.logger.error('Error while calculating: ' + sign.substring(0, sign.length - 2));
			} else {
			...
			if (typeof(answer.type) !== 'undefined' && typeof(answer.timestamp) !== 'undefined' && typeof(CALCULATION.binder[answer.timestamp]) !== 'undefined') {
				...
				widget.binders.trigger('onCalculate', {
					profiles: widget.service.cloneObj(CALCULATION.profiles),
	...
	var cargo = {
		collection: (typeof widget.options.get('goods') == 'object') ? widget.options.get('goods') : [],
		...
		add: function (item) {
			if (
				...
				widget.logger.error('Illegal item ' + item);
			} else {
		...
		get: function () {
			return widget.service.cloneObj(this.collection);
		}
	};
	...
	var LANG = {
		...
		get: function (wat) {
			if (typeof(this.collection[wat]) !== 'undefined') {
				...
				widget.logger.warn('No lang string with key ' + wat);
				return '';
			}
		},
		...
		write: function (data) {
			ipjq.getJSON(
				widget.options.get('templatepath'),
				{},
				HTML.save
			);
			...
			if (typeof(data.LANG) === 'undefined') {
				...
				LANG.collection = widget.service.cloneObj(data.LANG);
				loaders.onLANGLoad();
			}
		}
	};
	...
	var template = {
		...
		html: {
			get: function () {
				...
				return HTML.getBlock('widget', {
					'CITY': widget.options.get('defaultCity')
				});
			},

			makeADAPT: function () {
				if (widget.options.get('link')) {
					return;
				}
			...
			place: function () {
				...
				if (widget.options.get('link')) {
					ipjq('#' + widget.options.get('link')).html(html);
				} else if (widget.options.get('popup')) {
					widget.popupped = true;
				...
				if (!widget.options.get('choose')) {
					ipjq(IDS.get('cdek_widget_cnt')).addClass('nochoose');
				}
				var htmlka = HTML.getBlock('sidebar');

                if (widget.options.get('hidecash')) {
                    var temp = [];
                ...
                if (widget.options.get('hidedress')) {
                    var temp = [];
                    temp.push(htmlka.slice(0, ( widget.options.get('hidecash') ? 132:1349)));
                    temp.push(htmlka.slice(htmlka.indexOf('<hr>')));
                    htmlka = temp.join("");
				}
				
                if (widget.options.get('hidedelt')) {
		...
		controller: {
			...
			choosePVZ: function (id) {
				...
				widget.binders.trigger('onChoose', {
					'id': id,
					'PVZ': PVZ[id],
			...
			chooseCOURIER: function () {
				...
				widget.binders.trigger('onChooseProfile', {
					'id': 'courier',
					'city': DATA.city.current,
                    'cityName': DATA.city.getName(DATA.city.current),
			...
			open: function () {
				if (widget.options.get('link')) {
					widget.logger.error('This widget is in non-floating mode - link is set');
				} else {
					template.ui.open();
				}
			},
			...
			init: function (city) {
                ...
                if (city == false) {
                    ymaps.geolocation.get({}).then(function (result) {
                        ...
                        widget.options.set(city, 'defaultCity');
            ...
            loadMap: function (city) {
				...
				ymaps.geocode(city, {
					...
					if (!self.map) {
                        ...
                        self.map.events.add('boundschange', widget.hideLoader);
					} else {
				...
				if (typeof pvzList === 'object') {
					...
					for (var i in pvzList) {
						...
						pvzList[i].placeMark = new ymaps.Placemark([pvzList[i].cY, pvzList[i].cX], {}, {
							...
							iconImageHref: widget.options.get('path') + 'images/sdekNActive.png',
							iconImageSize: [40, 43],
						...
						pvzList[i].placeMark.events.add(['mouseenter'], function (metka) {
							...
							metka.get('target').options.set({iconImageHref: widget.options.get('path') + "images/sdekActive.png"});
						});

						pvzList[i].placeMark.events.add(['mouseleave'], function (metka) {
							if (template.ui.currentmark != metka.get('target')) {
								metka.get('target').listItem.removeClass('CDEK-widget__panel-list__item_active');
								metka.get('target').options.set({iconImageHref: widget.options.get('path') + "images/sdekNActive.png"});
							}
						});
			...
			blinkPVZ: function (event) {
				...
				if (event.data.link.readyToBlink) {
					...
					if (event.data.ifOn) {
						...
						cityPvz[event.data.id].placeMark.options.set({iconImageHref: widget.options.get('path') + "images/sdekActive.png"});
					} else {
						cityPvz[event.data.id].placeMark.options.set({iconImageHref: widget.options.get('path') + "images/sdekNActive.png"});
	...
	widget.binders.add(template.controller.updatePrices, 'onCalculate');

	widget.sdekSetPVZS = function (mtypes) {
		template.ymaps.clearMarks();
		if (typeof mtypes != 'undefined') {
	...
	widget.chooseCourier = function () {
		template.controller.chooseCOURIER();
	};

	widget.sdekWidgetEvents = function () {

		ipjq('.CDEK-widget__popup__close-btn').off('click').on('click', function () {
			ipjq(this).closest('.CDEK-widget__popup-mask').hide();
		});

		ipjq(IDS.get('cdek_widget_cnt')).on('click', '.CDEK-widget__sidebar-button', {widget: widget}, function () {
		...
		}).on('click', '.CDEK-widget__delivery-type__item', {widget: widget}, function (e) {
