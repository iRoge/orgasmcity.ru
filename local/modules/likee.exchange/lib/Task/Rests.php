...
class Rests extends Task
{
    ...
    /**
     * @public array Склады исключения
     */
    public $stores_exclude = [];
    ...
    /**
     * Проверка настройки на ежечасный импорт
     *
     * @throws ExchangeException
     */
    protected function checkHourlyImport()
    {
        $this->log("Проверяем тип импорта и разрешение на ежечасный импорт");
        ...
        $this->reader->on('only_changes', function ($reader, $xml) {
            ...
            if (defined("IMPORT_LOG") && IMPORT_LOG == "FULL") {
                $this->log($this->only_changes ? "Тип импорта: Only_changes" : "Тип импорта: Full");
            }
        });

        $this->reader->read();

        if ($this->only_changes == 1) {
            $this->log("Тип импорта: Only_changes");
        } else {
            $this->log("Тип импорта: Full");
        }
    }
    ...
    private function read()
    {
        $this->reader->on('rest', function ($reader, $xml) {
            ...
            if ($this->rests[$rest["offer_id"] . "-" . $rest["store_id"]]) {
                $this->log("Остаток " . $rest["offer_id"] . " магазина " . $rest["store_id"] . " дублируется");
            }

            $this->rests[$rest["offer_id"] . "-" . $rest["store_id"]] = [
    ...
    private function apply()
    {
        ...
        $this->log("Обновлено " . $countUpdate . " остатков");
        $this->log("Добавлено " . $countAdd . " остатков");

        if (!$this->only_changes) {
            /*file_put_contents(
                $_SERVER["DOCUMENT_ROOT"].'/illia.txt',
                print_r(array_keys($this->changes),1),
                FILE_APPEND
            );*/

            /*file_put_contents(
                $_SERVER["DOCUMENT_ROOT"].'/illia.txt',
                print_r(array_keys($this->quantity),1),
                FILE_APPEND
            );*/

            /*file_put_contents(
                $_SERVER["DOCUMENT_ROOT"].'/illia.txt',
                print_r(array_diff(array_keys($this->changes), array_keys($this->quantity)),1),
                FILE_APPEND
            );*/

            //$arDiff = array_diff(array_keys($this->changes), array_keys($this->quantity));

            /*foreach ($arDiff as $iProduct) {
                $rsQuantities = StoreProductTable::getList([
                    'filter' => [
                        'PRODUCT_ID' => $iProduct
                    ]
                ...
                while ($arQuantity = $rsQuantities->fetch()) {
                    StoreProductTable::delete($arQuantity['ID']);
                }
            }*/
