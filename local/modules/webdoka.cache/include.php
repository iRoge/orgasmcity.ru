<?
IncludeModuleLangFile(__FILE__);
/*CModule::AddAutoloadClasses(
    "webdoka.cache",
    array(
        "CWebdokaCacheAgent" => $_SERVER['DOCUMENT_ROOT']."/bitrix/modules/webdoka.cache/classes/general/cache.php",

    )
);*/
include_once  $_SERVER['DOCUMENT_ROOT']."/local/modules/webdoka.cache/classes/general/cache.php";




Class CWebdokaCache
{
    function OnBuildGlobalMenu(&$aGlobalMenu, &$aModuleMenu)
    {
        if($GLOBALS['APPLICATION']->GetGroupRight("main") < "R")
            return;

        $MODULE_ID = basename(dirname(__FILE__));
        $aMenu = array(
            "parent_menu" => "global_menu_services",
            "section" => $MODULE_ID,
            "sort" => 50,
            "text" => GetMessage("WD_AUTO"),
            "title" => '',
            //"url" => "partner_modules.php?module=".$MODULE_ID,
            "icon" => "fileman_sticker_icon",
            "page_icon" => "",
            "items_id" => $MODULE_ID."_items",
            "more_url" => array(),
            "items" => array()
        );







        if (file_exists($path = dirname(__FILE__).'/admin'))
        {
            if ($dir = opendir($path))
            {
                $arFiles = array();

                while(false !== $item = readdir($dir))
                {
                    if (in_array($item,array('.','..','menu.php')))
                        continue;

                    if (!file_exists($file = $_SERVER['DOCUMENT_ROOT'].'/bitrix/admin/'.$MODULE_ID.'_'.$item))
                        file_put_contents($file,'<'.'? require($_SERVER["DOCUMENT_ROOT"]."/local/modules/'.$MODULE_ID.'/admin/'.$item.'");?'.'>');

                    $arFiles[] = $item;
                }

                sort($arFiles);


                $arTitles = array(
                    'settings.php' => GetMessage("WD_NAST"),

                );


                foreach($arFiles as $key => $item) {
                    if($key == 0) {
                        $aMenu['items'][] = array(
                            'text' => $arTitles[$item],
                            'url' => $MODULE_ID.'_'.$item,
                            'module_id' => $MODULE_ID,
                            "title" => "",

                        );
                    }
                }
            }
        }
        $aModuleMenu[] = $aMenu;
    }



}




?>