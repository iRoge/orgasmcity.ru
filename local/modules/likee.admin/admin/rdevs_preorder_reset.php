...
if ($_SERVER['REQUEST_METHOD'] == "POST"
    ...
    if ($aItem && isset($aItem['ID'])) {
        ...
        LocalRedirect("/bitrix/admin/rdevs_preorder_reset.php?lang=".LANGUAGE_ID."&success=Y");
    } else {
        $strError .= 'В системе отсутствует свойство отключения доставки товара.<br>';
    }
}

$APPLICATION->SetTitle('Сброс предзаказа у товаров');

require($_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/main/include/prolog_admin_after.php");

if (!empty($_REQUEST['success'])) {
    CAdminMessage::ShowMessage(array(
        "MESSAGE" => 'Сброс предзаказа произведен успешно.',
        "TYPE" => 'OK',
        "HTML" => true
    ));
} elseif ($strError) {
...
?>
<form method="POST" action="rdevs_preorder_reset.php?lang=ru" enctype="multipart/form-data" name="editform">
    <?= bitrix_sessid_post() ?>
    <input type="hidden" name="lang" value="<?= LANG ?>">
    <div class="adm-detail-content-item-block">
        <input type="submit" name="db_reset_preorder" value="Сбросить предзаказ" class="adm-btn-save">
        <div class="adm-info-message-wrap">
            <div class="adm-info-message">
                <p>Всем артикулам, у которых установлено свойство предзаказ, данное значение будет сброшено.</p>
